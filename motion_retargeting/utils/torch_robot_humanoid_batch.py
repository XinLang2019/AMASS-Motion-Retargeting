import torch 
import numpy as np
import motion_retargeting.utils.rotation_conversions as tRot
...
class Humanoid_Batch:
    def __init__(self, robot_type='g1', mjcf_file=f"resources/robots/g1/g1_29dof_anneal_23dof.xml", extend_hand = True, extend_head = False, device = torch.device("cpu")):
        ...
        if extend_hand:
            ...
            if self.robot_type == 'g1':
                self._parents = torch.cat((mjcf_data['parent_indices'], torch.tensor([19, 23]))).to(device) # Adding the hands joints g1
                arm_length = 0.1
            elif self.robot_type == 'h1':
                self._parents = torch.cat((mjcf_data['parent_indices'], torch.tensor([15, 19]))).to(device) # Adding the hands joints h1
                arm_length = 0.3
    ...
    def fk_batch(self, pose, trans, convert_to_mat=True, return_full = False, dt=1/30):
        ...
        if self.robot_type == 'g1':
            if self.extend_hand and self.extend_head and pose.shape[-2] == 27:  
